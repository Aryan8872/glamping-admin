...
import { CampFormValues, Facility } from "../types/campTypes";
import { campSchema } from "@/lib/validation/campValidation";
...
export default function CampForm({
  ...
  const { errors, setErrors, clearAllErrors, checkField, setFieldError } =
    useFormValidation();
  ...
  const handleFieldChange = (field: keyof State, value: any) => {
    ...
    if (field in campSchema.shape) {
      checkField(
        field,
        value,
        campSchema.shape[field as keyof typeof campSchema.shape],
      );
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAvailableFacilities(Array.isArray(facilities) ? facilities : []);
        setAvailableHosts(Array.isArray(users) ? users : []);
        setAvailableAdventures(Array.isArray(adventures) ? adventures : []);
        setAvailableDestinations(
          Array.isArray(destinations) ? destinations : [],
        );
        setAvailableExperiences(Array.isArray(experiences) ? experiences : []);
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (state.images.length === 0 && state.newImages.length === 0) {
      setGeneralError("At least one camp image is required");
      window.scrollTo({ top: 0, behavior: "smooth" });
      return;
    }
    ...
    try {
      ...
      // Append Host
      formData.append("hostId", state.hostId ? state.hostId.toString() : "");
      ...
      formData.append(
        "destinationId",
        state.destinationId ? state.destinationId.toString() : "",
      );
      // Append Experiences
      formData.append(
        "experienceIds",
      ...
      if (typeof rawMessage === "string") {
        if (rawMessage.includes("Internal Server Error")) {
          ...
          errorMessage = "A camp with this name already exists.";
        } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onBlur={() =>
                  checkField("name", state.name, campSchema.shape.name)
                }
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleFieldChange("pricePerNight", e.target.value)
                }
                onBlur={() =>
                  checkField(
                    "pricePerNight",
                    state.pricePerNight,
                    campSchema.shape.pricePerNight,
                  )
                }
                className={`focus:outline-none focus:ring-2 focus:ring-blue-500 border ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) => handleFieldChange("maxAdult", e.target.value)}
                min="0"
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleFieldChange("maxChildren", e.target.value)
                }
                min="0"
                className="focus:outline-none focus:ring-2 focus:ring-blue-500 border border-gray-300 rounded-lg p-2.5 text-gray-700"
              />
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                type="number"
                value={state.maxPets}
                onChange={(e) => handleFieldChange("maxPets", e.target.value)}
                min="0"
                className="focus:outline-none focus:ring-2 focus:ring-blue-500 border border-gray-300 rounded-lg p-2.5 text-gray-700"
              />
            </div>
              ...
              return (
                ...
                >
                  <div className="w-10 h-10 rounded-md bg-gray-200 overflow-hidden flex-shrink-0">
                    {experience.imageUrl ? (
                      <img
                        src={buildImageUrl(experience.imageUrl)}
