...
import { ZodSchema, ZodError, ZodIssue, ZodObject } from "zod";
...
export default function GenericForm<T>({
  ...
  const handleBlur = (name: keyof T) => {
    if (validationSchema && validationSchema instanceof ZodObject) {
      const fieldSchema = validationSchema.shape[name as string];
      if (fieldSchema) {
        try {
          fieldSchema.parse(state.values[name]);
          // If valid, explicitly clear error (already clear on change, but safe to do here)
          dispatch({ type: "SET_FIELD_ERROR", field: name, error: undefined });
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.body?.errors) {
        const backendErrors: Partial<Record<keyof T, string>> = {};
        error.body.errors.forEach((err: any) => {
          if (err.field) {
            // Handle nested fields if necessary, or simple mapping
            backendErrors[err.field as keyof T] = err.message;
          }
        });
  ...
  return (
    <div className="w-full bg-white p-6 rounded-xl shadow-sm border border-gray-200">
      ...
      {state.generalError && (
        <div className="mb-6 p-4 bg-red-50 border border-red-100 text-red-600 text-sm font-semibold rounded-lg animate-pulse">
          {state.generalError}
        </div>
      )}
